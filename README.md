...
**NOTE**: *You can call your geonode project whatever you like following the naming conventions for python packages (generally lower case with underscores (``_``). In the examples below, replace ``stg_geonode`` with whatever you would like to name your project.*
...
1. Prepare the Environment
    ...
    django-admin startproject --template=./geonode-project -e py,sh,md,rst,json,yml,ini,env,sample,properties -n monitoring-cron -n Dockerfile stg_geonode
...
```bash
SOURCE_URL=$SOURCE_URL TARGET_URL=$TARGET_URL ./stg_geonode/br/backup.sh $BKP_FOLDER_NAME
```
...
```bash
docker exec -it django4stg_geonode sh -c 'SOURCE_URL=$SOURCE_URL TARGET_URL=$TARGET_URL ./stg_geonode/br/backup.sh $BKP_FOLDER_NAME'
```

### Run a Restore

```bash
SOURCE_URL=$SOURCE_URL TARGET_URL=$TARGET_URL ./stg_geonode/br/restore.sh $BKP_FOLDER_NAME
```
...
```bash
docker exec -it django4stg_geonode sh -c 'SOURCE_URL=$SOURCE_URL TARGET_URL=$TARGET_URL ./stg_geonode/br/restore.sh $BKP_FOLDER_NAME'
```
